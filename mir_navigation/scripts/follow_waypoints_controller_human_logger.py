...
#         self.declare_parameter('csv_dir', os.path.expanduser('~/report_ccu/dynamic_obstacle/human_straight_moving/DWB'))
...
class WaypointFollowerPathError(Node):
    ...
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter('csv_dir', os.path.expanduser('~/report_ccu/path_error/cont_DWB/'))
    ...
    def _build_csv_path(self):
        ...
        os.makedirs(csv_dir, exist_ok=True)
        return os.path.join(csv_dir, f'path_error_{ts}.csv')
    ...
    def get_result_callback(self, future):
        ...
        self.get_logger().info(f'Waypoint action finished with status {status}')
        self.stop_and_save()
        rclpy.shutdown()
    ...
    def stop_and_save(self):
        ...
        self.get_logger().info(f'Path error CSV written ({len(self._rows)} rows)')
