...
class NearestObstacleDistance(Node):
    def __init__(self):
        ...
        # --- Parameters ---
        self.declare_parameter('costmap_topic', '/local_costmap/costmap')  # Changed default
    ...
    def _on_occupancy_grid(self, msg: OccupancyGrid):
        ...
        min_idx = int(np.argmin(d_pix))
        nearest_pix_dist = float(d_pix[min_idx])
        dist_m = nearest_pix_dist * res

        # Subtract inscribed radius
        inscribed = self._get_inscribed_radius()
        clearance = max(0.0, dist_m - inscribed) if math.isfinite(dist_m) else float('inf')

        # Publish
        out = Float32()
        out.data = clearance if self.subtract_inscribed else dist_m
    ...
    # ... (keep your existing _get_inscribed_radius and other methods) ...
